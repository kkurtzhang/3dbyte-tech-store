...
// Get batch 3 products (skip first 50)
const batch3Products = allProducts.slice(99);
console.log(`Pushing descriptions for ${batch3Products.length} batch 3 products...`);
...
async function main() {
  ...
  for (const product of batch3Products) {
    const success = await createDescription(product);
    if (success) created++;
    await new Promise(r => setTimeout(r, 150));
  }
  
  console.log(`\nDone! Created ${created}/${batch3Products.length} descriptions`);
}
