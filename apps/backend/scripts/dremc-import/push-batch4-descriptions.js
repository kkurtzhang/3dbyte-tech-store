...
const batch4Products = allProducts.slice(149);
console.log(`Pushing descriptions for ${batch4Products.length} batch 4 products...`);
...
async function main() {
  ...
  for (const product of batch4Products) {
    const success = await createDescription(product);
    if (success) created++;
    await new Promise(r => setTimeout(r, 150));
  }
  
  console.log(`\nDone! Created ${created}/${batch4Products.length} descriptions`);
}
