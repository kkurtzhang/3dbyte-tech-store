const STRAPI_URL = 'http://192.168.0.45:1337';
const STRAPI_TOKEN = '';

async function fixStrapiHandles() {
  // Get all product descriptions with dremc- prefix
  const response = await fetch(`${STRAPI_URL}/api/product-descriptions?filters[product_handle][$startsWith]=dremc-&limit=100`, {
    headers: { 'Authorization': `Bearer ${STRAPI_TOKEN}` }
  });
  
  const data = await response.json();
  console.log(`Found ${data.data.length} records to fix in Strapi`);
  
  let updated = 0;
  
  for (const item of data.data) {
    const oldHandle = item.product_handle;
    const newHandle = oldHandle.replace('dremc-', '');
    const docId = item.documentId;
    
    const updateResponse = await fetch(`${STRAPI_URL}/api/product-descriptions/${docId}`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${STRAPI_TOKEN}`,
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({
        data: { product_handle: newHandle }
      })
    });
    
    const result = await updateResponse.json();
    
    if (result.data?.id) {
      console.log(`✅ ${oldHandle.substring(0, 35)}... → ${newHandle.substring(0, 35)}...`);
      updated++;
    } else {
      console.log(`❌ Failed: ${oldHandle}`);
    }
    
    await new Promise(r => setTimeout(r, 100));
  }
  
  console.log(`\nDone! Updated ${updated}/${data.data.length} Strapi handles`);
}

fixStrapiHandles().catch(console.error);
