...
export default async function importBatch5({ container }: ExecArgs) {
  const logger = container.resolve("logger");
  const productModuleService = container.resolve(Modules.PRODUCT);

  logger.info("=== IMPORTING DREMC BATCH 5 PRODUCTS ===");

  const dataPath = path.join(__dirname, "../../scripts/dremc-import/data/products-batch-5.json");
  const productsData = JSON.parse(fs.readFileSync(dataPath, "utf8"));
  const products = productsData.products;

  logger.info(`Loading ${products.length} products from batch 5...`);
