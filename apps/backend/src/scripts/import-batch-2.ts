...
export default async function importBatch2({ container }: ExecArgs) {
  const logger = container.resolve("logger");
  const productModuleService = container.resolve(Modules.PRODUCT);

  logger.info("=== IMPORTING DREMC BATCH 2 PRODUCTS ===");

  const dataPath = path.join(__dirname, "../../scripts/dremc-import/data/products-batch-2.json");
  const productsData = JSON.parse(fs.readFileSync(dataPath, "utf8"));
  const products = productsData.products;

  logger.info(`Loading ${products.length} products from batch 2...`);
  ...
  for (const product of products) {
    try {
      ...
      // NO dremc- prefix in handle!
      const newProduct = await productModuleService.createProducts({
        title: product.title,
        handle: product.handle, // Use original handle directly
