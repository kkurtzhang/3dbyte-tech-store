...
export default async function importBatch3({ container }: ExecArgs) {
  const logger = container.resolve("logger");
  const productModuleService = container.resolve(Modules.PRODUCT);

  logger.info("=== IMPORTING DREMC BATCH 3 PRODUCTS ===");

  const dataPath = path.join(__dirname, "../../scripts/dremc-import/data/products-batch-3.json");
  const productsData = JSON.parse(fs.readFileSync(dataPath, "utf8"));
  const products = productsData.products;

  logger.info(`Loading ${products.length} products from batch 3...`);
  ...
  for (const product of products) {
    try {
      ...
      // Clean handle - remove special characters
