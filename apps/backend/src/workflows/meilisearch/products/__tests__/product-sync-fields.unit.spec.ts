...
describe("MEILISEARCH_PRODUCT_SYNC_FIELDS", () => {
  ...
  it("requests calculated sale pricing fields for storefront sale indexing", () => {
    expect(fields).toContain("variants.prices.*");
    expect(fields).toContain("variants.calculated_price.*");
    expect(fields).toContain("variants.original_price");
    expect(fields).toContain("variants.original_price_calculated");
  });
