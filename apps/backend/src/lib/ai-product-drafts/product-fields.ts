...
export function isFieldApplicable(kind: ProductKind, field: FactField) {
  ...
  if (field === "nozzle_diameter_mm" || field === "thread")
    return ["nozzle", "hotend"].includes(kind)
  ...
  if (field === "max_temperature_c")
    return ["hotend", "nozzle", "build_plate", "tool"].includes(kind)
  return true
}
...
const range = z
  .object({
    min: z.number().min(-100).max(1000),
    max: z.number().min(-100).max(1000),
  })
  .strict()
...
export const factValueSchemas = {
  ...
  requires_enclosure: z.boolean(),
  requires_hardened_nozzle: z.boolean(),
  drying_recommended: z.boolean(),
  compatible_printers: strings,
  compatible_build_surfaces: strings,
  voltage_v: z.number().positive().max(1000),
  current_a: z.number().positive().max(1000),
    ...
    if (fact.applicability !== "known") {
      if (fact.value !== null)
        ctx.addIssue({
          code: "custom",
          path: ["value"],
          message: "Unknown or inapplicable facts must have a null value",
        })
    } else if (!factValueSchemas[fact.field].safeParse(fact.value).success) {
      ctx.addIssue({
        code: "custom",
        path: ["value"],
        message: "Invalid value or unit for this fact",
      })
    }
  })
