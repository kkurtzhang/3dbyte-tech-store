...
const prepareReplacement = (draft: unknown, body: unknown) => require("../reprocess").prepareReplacement(draft, body)

describe("replace research safely", () => {
  const draft = { id: "old", status: "needs_review", product_input: packetV3.product_input, raw_packet: { packet_version: 2 } }
  ...
  it("rejects stale review, different product, legacy payload, and any imported work", () => {
    for (const [record, request] of [
      ...
      [draft, { ...body, packet: { ...packetV3, product_input: { ...packetV3.product_input, product_name: "Other product" } } }],
      [draft, { ...body, packet: { packet_version: 2 } }],
      [{ ...draft, status: "imported" }, body],
      [{ ...draft, status: "approved" }, body],
      [{ ...draft, import_progress: { medusa_product: { status: "completed" } } }, body],
