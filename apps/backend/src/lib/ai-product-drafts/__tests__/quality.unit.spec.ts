...
const assessAiProductDraftQuality = (draft: unknown) =>
  require("../quality").assessAiProductDraftQuality(draft)
...
describe("AI draft evidence and category quality", () => {
  it("does not label a legacy hotend as filament or promote its maximum to print settings", () => {
    const packet = ProductResearchPacketSchema.parse({
      ...legacy,
      product_input: {
        ...legacy.product_input,
        product_name: "Creality K1C OEM Ceramic Hotend Kit",
      },
      facts: {
        ...legacy.facts,
        recommended_nozzle_temp_c: {
          min: 0,
          max: 300,
          source_url: legacy.sources[0].url,
    ...
    expect(
      draft.metadata.three_d_printing?.recommended_nozzle_temp_c
    ).toBeUndefined()
  ...
  it("normalizes sourced hotend facts without filament-only fields", () => {
    const draft = normalizeProductResearchPacket(
    ...
    expect(draft.product_document_suggestions[0]).toMatchObject({
      document_type: "manual",
      source_kind: "official_manual",
    })
    const quality = assessAiProductDraftQuality({
      raw_packet: packetV3,
      normalized_draft: draft,
    })
    expect(quality.can_approve).toBe(true)
    const changes = buildAiProductDraftChangeSet({
      current_product: { id: "new" },
      normalized_draft: draft,
    })
    expect(changes).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ path: "metadata.ai_core.product_kind" }),
      ])
    )
  })
  ...
  it("does not give empty copy a default confidence or approval eligibility", () => {
    ...
    const draft = normalizeProductResearchPacket(
      ProductResearchPacketSchema.parse(packet)
    )
  ...
  it("blocks missing evidence, background sources, caveats, and unrelated category fields", () => {
    for (const change of [
      ...
      {
        facts: packetV3.facts.map((f) => ({
          ...f,
          warning: "Inferred from another model",
        })),
      },
      {
        facts: [
          ...
          {
            ...packetV3.facts[0],
            field: "recommended_nozzle_temp_c",
  ...
  it("rejects reversed ranges and metadata tampering", () => {
    expect(
      ProductResearchPacketSchema.safeParse({
        ...packetV3,
        facts: [
          {
            ...packetV3.facts[0],
            field: "recommended_nozzle_temp_c",
            value: { min: 300, max: 200 },
          },
        ],
      }).success
    ).toBe(false)
