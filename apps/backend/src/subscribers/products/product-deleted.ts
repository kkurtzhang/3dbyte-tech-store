...
export default async function productDeletedHandler({
  ...
  try {
    ...
    logger.info(`Processing product.deleted event for product: ${data.id}`);

    // Delete product description in Strapi
    await strapiService.deleteProductDescription(data.id);

  } catch (error) {
    logger.error(
      `Failed to handle product.deleted event for ${data.id}`,
