...
import { syncBrandToStrapiWorkflow } from "../../workflows/meilisearch/brands/sync-brand-to-strapi-workflow"
import { BRAND_MODULE } from "../../modules/brand"
import type BrandModuleService from "../../modules/brand/service"
import type { SyncBrandsStepBrand } from "@3dbyte-tech-store/shared-types"

/**
 * Brand Created Event Handler
 *
 * Triggered when a new brand is created in Medusa.
...
export default async function brandCreatedHandler({
  ...
  try {
    logger.info(`Processing brand.created event for brand: ${data.id}`)
    ...
    logger.info(`Successfully processed brand.created for brand: ${data.id}`)
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error"
    logger.error(`Failed to handle brand.created event for ${data.id}: ${message}`)
