...
const normalizeLineItemForStoreDisplay = (
  ...
  const exclusiveSubtotal =
    displayTotal !== null && taxRate > 0
      ? displayTotal / (1 + taxRate)
      : displayTotal;
...
const normalizeOrderForStoreDisplay = (order: LookupOrder): LookupOrder => {
  ...
  if (!isAdditiveTaxTotal) return order;

  const discountTotal = getFiniteNumber(order.discount_total) ?? 0;
  const shippingTotal =
    getFiniteNumber(order.shipping_subtotal) ??
    getFiniteNumber(order.shipping_total) ??
    0;
