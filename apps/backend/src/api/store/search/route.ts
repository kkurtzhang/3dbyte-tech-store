...
export const GET = async (
  ...
  const knex = req.scope.resolve<Knex<any, any[]>>('__pg_connection__');

  const engine = new SearchEngine(knex);

  // `fields` comes from validateAndTransformQuery / createFindParams
  const select =
    (req.queryConfig?.fields as string[] | undefined) ?? []
