  ...
  try {
    ...
    if (action === 'add') {
      ...
      if (!parsedBody.success) {
        return NextResponse.json(
          { success: false, error: 'Invalid address payload' },
          { status: 400 }
        )
      }
      const { customer } = await sdk.store.customer.createAddress(parsedBody.data, {}, authHeaders)
    ...
    if (action === 'delete' && parsedAddressId.success) {
      await sdk.store.customer.deleteAddress(parsedAddressId.data, authHeaders)
