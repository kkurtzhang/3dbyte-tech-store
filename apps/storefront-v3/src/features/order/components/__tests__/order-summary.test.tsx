...
describe('OrderSummary', () => {
  ...
  it('groups bundled items as a single purchased line with variant-aware includes', () => {
    ...
    expect(screen.getByText(/Some items in this order are pre-ordered/i)).toBeInTheDocument()
  })
  ...
  it('shows shipment tracking numbers from fulfillment data and labels', () => {
    render(
      <OrderSummary
        order={createOrder({
          ...
          fulfillments: [
            {
              ...
              labels: [
                {
                  id: 'fl_1',
                  tracking_number: 'STG-456',
                  tracking_url: 'https://tracking.example/STG-456',
                },
              ],
            },
          ],
    ...
    expect(screen.getByText('STG-123')).toBeInTheDocument()
    expect(screen.getByText('STG-456')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: 'STG-123' })).toHaveAttribute(
      'href',
      'https://tracking.example/STG-123'
    )
  })
})
