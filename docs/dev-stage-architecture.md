...
- Strapi Meilisearch plugin persists credentials in `strapi_core_store_settings`. If `MEILISEARCH_CMS_API_KEY` is removed from env, the old stored key may survive and keep breaking sync/admin operations until cleared.
- CMS bootstrap clears `plugin_meilisearch_meilisearch_api_key` automatically when `MEILISEARCH_CMS_API_KEY` is empty, so stale restored/store values do not linger.
