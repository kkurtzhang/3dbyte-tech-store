...
- Status: implemented in `feature/langfuse-assistant-tracing`; awaiting staging deploy verification.
...
- Deliverable:
  ...
  - Group turns from one browser chat conversation with a Langfuse session id generated by the storefront drawer.
  - Track DeepSeek cache-hit/cache-miss input tokens with Langfuse usage detail keys `input_cache_hit_tokens` and `input_cache_miss_tokens`.
...
- Acceptance:
  ...
  - Multiple turns in one browser drawer appear under the same Langfuse session.
  - DeepSeek V4 Flash generations show cache-aware token usage and cost after the custom Langfuse model pricing is configured.
